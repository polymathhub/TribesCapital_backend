...
export default function MessagingPage({ user }) {
  ...
  const getConversationTitle = useCallback((conversation) => { if (!conversation) return 'Conversation'; if (conversation.type === 'DIRECT') { const peerMember = conversation.members?.find((member) => member.userId !== user?.id && member.user?.id !== user?.id); const peer = peerMember?.user || peerMember; const peerName = displayName(peer); return peerName !== 'Member' ? peerName : conversation.title || 'Direct message'; } return conversation.title || conversation.channel?.name || 'Conversation'; }, [user?.id]);
  ...
  useEffect(() => {
    ...
    messagingMembersAPI.list({ query: mentionQuery || undefined, limit: 8 }).then((response) => {
      if (!cancelled) setMentionResults(response?.data?.data ?? response?.data ?? []);
  ...
  useEffect(() => {
    ...
    messagingMembersAPI.list({ query: peopleSearchQuery.trim(), limit: 12 }).then((response) => {
      if (!cancelled) setPeopleSearchResults(response?.data?.data ?? response?.data ?? []);
  ...
  return <main className="messaging-page">
    ...
    <div className={`messaging-layout mobile-${mobileView}`}>
      ...
      <section className="thread-panel">{!selectedConversation ? <div className="thread-empty"><span className="empty-icon"><HugeiconsIcon icon={BubbleChatIcon} size={22} color="currentColor" strokeWidth={1.8} /></span><h2>Choose a conversation</h2><p>Select a conversation from your inbox or start a new one.</p><button type="button" className="messaging-primary-button" onClick={() => openNewConversation('direct')}>Start a conversation</button></div> : <>
        ...
        {mentionQuery !== null && mentionResults.length > 0 && <div className="mention-suggestions" role="listbox" aria-label="Mention a user">{mentionResults.map((person) => <button type="button" key={person.id} onMouseDown={(event) => event.preventDefault()} onClick={() => selectMention(person)}><Avatar person={person} size="sm" /><span><strong>{displayName(person)}</strong><small>{person.email || 'Workspace member'}</small></span></button>)}</div>}
