...
function Avatar({ person, label = 'Member', size = 'md' }) {
  return <span className={`message-avatar message-avatar-${size}`} title={displayName(person) || label}>
...
function MessageRow({ message, user, onReply, onEdit, onDelete, onReact, onRetry }) {
  const mine = message.senderId === user?.id;
  const [menuOpen, setMenuOpen] = useState(false);
  const reactions = Array.isArray(message.reactions) ? message.reactions : [];
  const groupedReactions = reactions.reduce((groups, item) => ({ ...groups, [item.reaction]: [...(groups[item.reaction] || []), item] }), {});
  return <article className={`message-row ${mine ? 'mine' : ''} ${message.sendFailed ? 'send-failed' : ''}`}>
    ...
    <div className="message-content-wrap">
      ...
      <div className={`message-bubble ${message.isDeleted ? 'deleted' : ''}`}>
        ...
        {message.attachments?.length > 0 && <div className="message-attachments">{message.attachments.map((attachment) => <a key={attachment.id || attachment.url} href={attachment.url === '#' ? undefined : attachment.url} target="_blank" rel="noreferrer" className={`attachment-card ${attachment.url === '#' ? 'pending' : ''}`}><span className="attachment-icon"><Icon name="file" size={16} /></span><span><strong>{attachment.fileName}</strong><small>{attachment.mimeType || 'Attachment'}</small></span></a>)}</div>}
      </div>
      {message.sendFailed && <button type="button" className="message-retry" onClick={() => onRetry(message)}>Retry sending</button>}
      {Object.keys(groupedReactions).length > 0 && <div className="reaction-list">{Object.entries(groupedReactions).map(([reaction, items]) => <button key={reaction} type="button" className="reaction-chip" onClick={() => onReact(message.id, reaction)}><span>{reaction}</span><small>{items.length}</small></button>)}</div>}
    </div>
...
export default function MessagingPage({ user }) {
  ...
  const unreadByConversation = useMemo(() => new Map(unread.map((item) => [item.conversationId, item.count || 0])), [unread]);
  ...
  const visibleConversations = useMemo(() => conversations.filter((conversation) => {
    if (conversationFilter === 'unread') return (unreadByConversation.get(conversation.id) || 0) > 0;
    if (conversationFilter === 'channels') return conversation.type !== 'DIRECT';
    return true;
  }), [conversationFilter, conversations, unreadByConversation]);
  ...
  const refreshUnread = useCallback(async () => { try { setUnread(unwrap(await messagingAPI.unread()) || []); } catch { /* supplementary */ } }, []);
  const loadConversations = useCallback(async () => {
    try { const next = unwrap(await messagingAPI.listConversations()); const list = Array.isArray(next) ? next : []; setConversations(list); setSelectedId((current) => current || list[0]?.id || null); }
    catch (requestError) { setError(requestError.response?.data?.message || 'Unable to load conversations.'); }
    finally { setLoading(false); }
  }, []);
  const loadActiveUsers = useCallback(async () => { try { const next = unwrap(await messagingAPI.listActiveUsers()); setActiveUsers(Array.isArray(next) ? next : []); } catch { /* supplementary */ } }, []);
  const loadMessages = useCallback(async (conversationId) => {
    if (!conversationId) return;
    try { const next = unwrap(await messagingAPI.getMessages(conversationId)); const list = Array.isArray(next) ? next : []; setMessages(list); if (list.length) { await messagingAPI.markRead(list[list.length - 1].id, list.map((message) => message.id)); await refreshUnread(); } }
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => setConnected(true)); socket.on('disconnect', () => setConnected(false)); socket.on('user:online', () => void loadActiveUsers()); socket.on('user:offline', () => void loadActiveUsers());
    socket.on('message:new', (message) => { if (message.conversationId !== selectedIdRef.current) { if (message.senderId !== user?.id) void refreshUnread(); return; } setMessages((current) => { const existing = current.find((item) => item.id === message.id || item.tempId === message.id); if (existing) return current.map((item) => item.id === existing.id ? { ...message, tempId: item.tempId } : item); return [...current, message]; }); if (message.senderId !== user?.id) void refreshUnread(); void loadConversations(); });
  ...
  useEffect(() => { if (!selectedId) return; void loadMessages(selectedId); const socket = socketRef.current; if (socket?.connected) socket.emit('conversation:join', { conversationId: selectedId }); setReplyingTo(null); setEditingMessage(null); setMobileView('thread'); }, [loadMessages, selectedId]);
  useEffect(() => { messageEndRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' }); }, [messages.length]);
  useEffect(() => { if (composerRef.current) { composerRef.current.style.height = 'auto'; composerRef.current.style.height = `${Math.min(composerRef.current.scrollHeight, 140)}px`; } }, [draft]);
  ...
  const createConversation = async (type = 'COMMUNITY_CHANNEL', targetUser = null) => { try { const payload = type === 'DIRECT' ? { type: 'DIRECT', participantIds: [targetUser.id], title: displayName(targetUser) } : { type: 'COMMUNITY_CHANNEL', title: 'Community conversation', channelName: 'Community conversation', isPrivateChannel: false }; const conversation = unwrap(await messagingAPI.createConversation(payload)); setConversations((current) => [conversation, ...current.filter((item) => item.id !== conversation.id)]); setSelectedId(conversation.id); } catch (requestError) { setError(requestError.response?.data?.message || 'Unable to create conversation.'); } };
  const startDirectMessage = async (person) => { if (!person || person.id === user?.id) return; const existing = conversations.find((conversation) => conversation.type === 'DIRECT' && conversation.members?.some((member) => member.userId === person.id)); if (existing) return chooseConversation(existing.id); await createConversation('DIRECT', person); };
  const sendMessage = async (event, retryMessage = null) => {
    event?.preventDefault(); const file = retryMessage?.pendingFile || pendingFile; const content = retryMessage?.content || draft.trim(); const conversationId = retryMessage?.conversationId || selectedId; if (!conversationId || (!content && !file) || editingMessage) return;
    const optimistic = retryMessage || { id: `local-${Date.now()}-${Math.random().toString(16).slice(2)}`, tempId: `local-${Date.now()}`, conversationId, senderId: user?.id, content: content || file?.name || '', createdAt: new Date().toISOString(), isOptimistic: true, sender: user, attachments: file ? [{ id: `local-file-${Date.now()}`, fileName: file.name, mimeType: file.type, size: file.size, url: '#' }] : [], replyTo: replyingTo };
    if (!retryMessage) { setMessages((current) => [...current, optimistic]); setDraft(''); setPendingFile(null); setReplyingTo(null); } socketRef.current?.emit('typing:stop', { conversationId });
    try { let attachments = []; if (file) { const formData = new FormData(); formData.append('file', file); attachments = [unwrap(await messagingAPI.uploadAttachment(conversationId, formData))]; } const payload = { content, attachments, replyToId: optimistic.replyTo?.id }; if (socketRef.current?.connected) socketRef.current.emit('message:send', { conversationId, ...payload }); else { const sent = unwrap(await messagingAPI.sendMessage(conversationId, payload)); setMessages((current) => current.map((item) => item.id === optimistic.id ? { ...sent, tempId: item.tempId } : item)); } if (retryMessage) setMessages((current) => current.filter((item) => item.id !== retryMessage.id)); }
    catch (requestError) { setMessages((current) => current.map((item) => item.id === optimistic.id ? { ...item, isOptimistic: false, sendFailed: true, pendingFile: file } : item)); setError(requestError.response?.data?.message || 'Unable to send message. Try again.'); }
  };
    ...
    <div className={`messaging-layout mobile-${mobileView}`}>
      <aside className="conversation-panel">
        ...
        <div className="active-user-panel">
          <div className="people-panel-header"><div><span className="panel-title">People</span><span className="panel-count">{activeUsers.length}</span></div><span className="people-live-label"><i /> {activeUsers.length ? 'Online now' : 'No one online'}</span></div>
          <div className="people-online-strip" aria-label="People online">{activeUsers.slice(0, 8).map((person) => <button className="people-online-avatar" key={person.id} type="button" onClick={() => void startDirectMessage(person)} title={`Message ${displayName(person)}`}><span className="people-avatar-wrap"><Avatar person={person} size="sm" /><i /></span></button>)}{activeUsers.length === 0 && <span className="people-empty-copy">Your online teammates will appear here.</span>}</div>
          <div className="people-online-list">{activeUsers.slice(0, 4).map((person) => <button className="active-user-item" key={person.id} type="button" onClick={() => void startDirectMessage(person)}><span className="people-row-avatar"><Avatar person={person} size="sm" /><i /></span><span><strong>{displayName(person)}</strong><small>Available to message</small></span><span className="people-dm-hint">DM</span></button>)}</div>
      ...
      <section className="thread-panel">
        {!selectedConversation ? <div className="thread-empty"><span className="empty-icon">✦</span><h2>Choose a conversation</h2><p>Select a conversation from your inbox or start a new one.</p></div> : <>
          <header className="thread-header"><button className="mobile-back" type="button" onClick={() => setMobileView('list')} aria-label="Back to conversations">←</button><Avatar person={selectedConversation.members?.find((member) => member.userId !== user?.id)?.user} /><div className="thread-header-copy"><h2>{getConversationTitle(selectedConversation)}</h2><p>{selectedConversation.type === 'DIRECT' ? 'Direct message' : 'Community conversation'} · {connected ? 'Live updates on' : 'Offline mode'}</p></div><div className="thread-header-actions"><button type="button" title="Search conversation">⌕</button><button type="button" title="Conversation details">ⓘ</button></div></header>
          <div className="message-list" ref={dropZoneRef} onDragOver={(event) => { event.preventDefault(); dropZoneRef.current?.classList.add('dragging'); }} onDragLeave={() => dropZoneRef.current?.classList.remove('dragging')} onDrop={handleDrop}>
            {messages.length === 0 ? <div className="thread-empty compact"><h3>No messages yet</h3><p>Start the conversation with a clear, human message.</p></div> : messages.map((message) => <MessageRow key={message.id || message.tempId} message={message} user={user} onReply={setReplyingTo} onEdit={(item) => { setEditingMessage(item); setDraft(item.content || ''); setTimeout(() => composerRef.current?.focus(), 0); }} onDelete={deleteMessage} onReact={reactToMessage} onRetry={(item) => void sendMessage(null, item)} />)}
            {typingNames.length > 0 && <div className="typing-indicator"><span className="typing-dots"><i /><i /><i /></span>{typingNames.length === 1 ? `${typingNames[0]} is typing…` : `${typingNames.slice(0, 2).join(', ')} are typing…`}</div>}<div ref={messageEndRef} />
          </div>
          ...
          {pendingFile && <div className="file-preview"><span>📎</span><strong>{pendingFile.name}</strong><small>{Math.max(1, Math.round(pendingFile.size / 1024))} KB</small><button type="button" onClick={() => setPendingFile(null)}>×</button></div>}
          <form className="message-composer" onSubmit={editingMessage ? submitEdit : sendMessage} onPaste={handlePaste}>
            <button type="button" className="composer-tool" onClick={() => fileInputRef.current?.click()} aria-label="Attach a file" title="Attach a file">＋</button><input ref={fileInputRef} type="file" hidden onChange={(event) => acceptFile(event.target.files?.[0])} />
            ...
            <button type="button" className="composer-tool" onClick={() => setDraft((value) => `${value}${value ? ' ' : ''}👍`)} aria-label="Add reaction">☺</button><button type="submit" className="send-button" disabled={!draft.trim() && !pendingFile} aria-label={editingMessage ? 'Save message' : 'Send message'}>{editingMessage ? 'Save' : 'Send'}</button>
          </form>
          <div className="composer-hint">Enter to send · Shift + Enter for a new line · Drop or paste an image to attach</div>
        </>}
      </section>
    </div>
  </main>;
