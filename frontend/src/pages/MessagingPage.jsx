...
export default function MessagingPage({ user }) {
  ...
  const refreshUnread = useCallback(async () => {
    try {
      setUnread(unwrap(await messagingAPI.unread()) || []);
    } catch {
      // Unread state should never block messaging.
    }
  }, []);
  ...
  const loadActiveUsers = useCallback(async () => {
    try {
      ...
      // Presence is supplementary and should not interrupt the inbox.
    }
  }, []);
  ...
  useEffect(() => {
    ...
    socket.on('message:new', (message) => {
      setMessages((current) => {
        if (message.conversationId !== selectedId) return current;
    ...
    socket.on('message:reaction', (reaction) => {
      setMessages((current) => current.map((item) => item.id === reaction.messageId ? { ...item, reactions: [...(item.reactions || []).filter((entry) => !(entry.userId === reaction.userId && entry.reaction === reaction.reaction)), reaction] } : item));
    });
  ...
  }, [loadActiveUsers, loadConversations, refreshUnread, selectedId, user?.id]);
  ...
  const sendMessage = async (event) => {
    ...
    const optimistic = {
      ...
      isOptimistic: true,
      sender: user,
      attachments: file ? [{ id: `${tempId}-file`, fileName: file.name, mimeType: file.type, size: file.size, url: '#' }] : [],
      replyTo: replyingTo,
    };
    ...
    try {
      ...
      if (socketRef.current?.connected) {
        socketRef.current.emit('message:send', { conversationId: selectedId, ...payload });
      } else {
        const sent = unwrap(await messagingAPI.sendMessage(selectedId, payload));
        setMessages((current) => current.map((item) => item.id === tempId ? { ...sent, tempId } : item));
      }
    } catch (requestError) {
  ...
  const handleComposerKeyDown = (event) => {
    if (event.key === 'Enter' && !event.shiftKey) {
      event.preventDefault();
      if (editingMessage) submitEdit(); else void sendMessage(event);
    }
  };
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const results = unwrap(await messagingAPI.search(value));
  ...
  return (
    <main className="messaging-page">
      <header className="messaging-header">
        <div className="messaging-heading-copy">
          <div className="messaging-title-row"><span className="messaging-mark"><HugeiconsIcon icon={BubbleChatIcon} size={24} color="currentColor" strokeWidth={1.8} /></span><div><h1>Messages</h1><p>Focused conversations for teams, communities, and direct collaboration.</p></div></div>
        </div>
      ...
      <div className={`messaging-layout mobile-${mobileView}`}>
        <aside className="conversation-panel">
          ...
          <div className="conversation-list">
            {loading ? <div className="empty-state">Loading conversations…</div> : conversations.length === 0 ? <div className="empty-state rail-empty"><strong>No conversations yet</strong><span>Start a conversation to get the discussion moving.</span></div> : conversations.map((conversation) => {
              const count = unread.find((item) => item.conversationId === conversation.id)?.count || 0;
