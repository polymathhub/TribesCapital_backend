...
export default function MessagingPage({ user }) {
  ...
  const loadMessages = useCallback(async (conversationId) => {
    ...
    try {
      ...
      const list = Array.isArray(next) ? next : [];
      setMessages(list);
      if (list.length) { await messagingAPI.markRead(list[list.length - 1].id, list.map((message) => message.id)); await refreshUnread(); }
  ...
  useEffect(() => {
    ...
    socket.on('message:new', (message) => {
      ...
      setMessages((current) => { const existing = current.find((item) => item.id === message.id || item.tempId === message.id); if (existing) return current.map((item) => item.id === existing.id ? { ...message, tempId: item.tempId } : item); return [...current, message]; });
  ...
  useEffect(() => {
    ...
    setReplyingTo(null); setEditingMessage(null); setMobileView('thread');
  ...
  const createConversation = async (type = 'COMMUNITY_CHANNEL', targetUser = null) => {
    try {
      ...
      setConversations((current) => [conversation, ...current.filter((item) => item.id !== conversation.id)]); setSelectedId(conversation.id);
  ...
  const sendMessage = async (event, retryMessage = null) => {
    ...
    const file = retryMessage?.pendingFile || pendingFile;
    const content = retryMessage?.content || draft.trim();
    const conversationId = retryMessage?.conversationId || selectedId;
    ...
    if (!retryMessage) { setMessages((current) => [...current, optimistic]); setDraft(''); setPendingFile(null); setReplyingTo(null); }
    socketRef.current?.emit('typing:stop', { conversationId });
    try {
      let attachments = [];
      if (file) { const formData = new FormData(); formData.append('file', file); attachments = [unwrap(await messagingAPI.uploadAttachment(conversationId, formData))]; }
      const payload = { content, attachments, replyToId: optimistic.replyTo?.id };
      if (socketRef.current?.connected) socketRef.current.emit('message:send', { conversationId, ...payload });
      else { const sent = unwrap(await messagingAPI.sendMessage(conversationId, payload)); setMessages((current) => current.map((item) => item.id === optimistic.id ? { ...sent, tempId: item.tempId } : item)); }
      if (retryMessage) setMessages((current) => current.filter((item) => item.id !== retryMessage.id));
    } catch (requestError) {
      setMessages((current) => current.map((item) => item.id === optimistic.id ? { ...item, isOptimistic: false, sendFailed: true, pendingFile: file } : item));
