  ...
  return (
    ...
    >
      ...
      <div
        style={{
          flex: 1,
          padding: isMobile ? '32px 24px 24px' : isTablet ? '40px 44px' : '48px 52px',
          display: 'flex',
...
function LoginPage({ onNavigate, onLoginSuccess }) {
  ...
  const handleGoogleSignIn = async () => {
    try {
          ...
          callback: async (response) => {
            try {
              if (!response.credential) {
                reject(new Error('Failed to get credential from Google'));
                return;
              }
              ...
              if (result?.data?.accessToken) {
                ...
                reject(new Error('Authentication failed. Invalid response from server.'));
              }
            } catch (err) {
      ...
      setError(err.message || 'Google sign-in error: Unknown error');
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <input
            ...
            disabled={loading}
            style={{ width: isMobile ? 14 : 16, height: isMobile ? 14 : 16, cursor: 'pointer' }}
          />
    ...
    try {
      ...
      const tokenPromise = new Promise((resolve, reject) => {
        window.google.accounts.id.requestIdToken({
          ...
          error_callback: () => {
            reject(new Error('Google sign-up was cancelled or failed'));
          },
        });
      });
      ...
      setError(err.message || 'Google sign-up error: Unknown error');
      console.error('Google sign-up error:', err);
      setLoading(false);
    }
  };
...
function ForgotPasswordPage({ onNavigate, onLoginSuccess }) {
  const [email, setEmail] = useState('');
  ...
  return (
    <TwoCol>
      <div style={{ marginBottom: 28 }}>
        <LogoFull size="medium" animate={false} />
      </div>
      <h1 style={{ fontSize: isMobile ? 22 : 28, fontWeight: 700, color: C.text, margin: '0 0 6px', letterSpacing: -0.5 }}>
        Reset your password
      </h1>
      <p style={{ fontSize: 14, color: C.textGray, marginBottom: isMobile ? 20 : 28, lineHeight: 1.5 }}>
        {step === 'email' ? 'Enter your email to receive a password reset code' : 'Enter the code and your new password'}
      </p>
      ...
      {message && (
        <div style={{ background: '#DCFCE7', border: `1px solid ${C.success}`, borderRadius: 8, padding: 12, marginBottom: 16, fontSize: 13, color: C.success }}>
          {message}
        </div>
      )}
      ...
      {step === 'email' && (
        <>
          ...
          <Btn onClick={handleSendCode} loading={loading} disabled={loading}>
      ...
      {step === 'code' && (
        <>
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                style={{
                  ...baseInput,
                  paddingRight: 44,
                  borderColor: focusConfirm ? C.primaryMid : C.border,
                  boxShadow: focusConfirm ? `0 0 0 3px rgba(124,58,237,0.12)` : 'none',
                  opacity: loading ? 0.6 : 1,
                }}
              />
              <button
          ...
          <Btn onClick={handleResetPassword} loading={loading} disabled={loading}>
            {loading ? 'Resetting…' : 'Reset Password'}
          </Btn>
        </>
      )}

      <p style={{ textAlign: 'center', fontSize: 13, color: C.textGray, marginTop: 22 }}>
        <button
          onClick={() => {
            setStep('email');
            setEmail('');
  ...
  return (
    <div
      style={{
        ...
        minHeight: '100vh',
        background: '#F9FAFB',
        padding: isMobile ? '20px 20px 0' : '20px',
    ...
    >
      ...
      {page === 'forgot' && <ForgotPasswordPage onNavigate={setPage} onLoginSuccess={handleLoginSuccess} />}
    </div>
  );
}
