...
describe('MessagingGateway', () => {
  ...
  const makeGateway = () => { const gateway = new MessagingGateway(jwtService as any, configService as any, prisma as any, messagingService as any); const emit = jest.fn(); const serverEmit = jest.fn(); const room = jest.fn(() => ({ emit })); (gateway as any).server = { to: room, emit: serverEmit }; return { gateway, emit, serverEmit, room }; };
  beforeEach(() => jest.clearAllMocks());

  it('sends new messages only to the conversation room and preserves client identity', async () => {
    const { gateway, room, serverEmit } = makeGateway();
    ...
    expect(serverEmit).not.toHaveBeenCalledWith('message:new', expect.anything());
  });
  ...
  it('tracks presence per socket and only broadcasts offline after the final socket closes', async () => {
    const { gateway, serverEmit } = makeGateway();
    jwtService.verifyAsync.mockResolvedValue({ sub: 'user-1' });
    await gateway.handleConnection({ id: 'socket-1', data: {}, handshake: { auth: { token: 'token-1' } }, join: jest.fn() } as any);
    ...
    expect(serverEmit).toHaveBeenCalledWith('user:offline', { userId: 'user-1' });
  });
  ...
  it('accepts presence heartbeats and can restore a stale session', async () => {
    ...
    expect(serverEmit).toHaveBeenCalledWith('user:online', { userId: 'user-1' });
  });
