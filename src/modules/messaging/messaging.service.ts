...
export class MessagingService {
  ...
  async trackUserOnline(userId: string, socketId: string) {
    ...
    await this.prisma.messagingPresenceSession.deleteMany({ where: { socketId } });
    const existing = await this.prisma.messagingPresenceSession.count({ where: { userId } });
  ...
  async trackUserHeartbeat(userId: string, socketId: string) {
    ...
    await this.prisma.messagingPresenceSession.updateMany({
  ...
  async getUnreadCounts(userId: string) {
    ...
    const grouped = await this.prisma.message.groupBy({
      by: ['conversationId'],
      where: { conversationId: { in: conversationIds }, senderId: { not: userId }, isDeleted: false, reads: { none: { userId } } },
      _count: { _all: true },
    });
  ...
  async getConversation(userId: string, conversationId: string) {
    ...
    return this.prisma.conversation.findUnique({
      where: { id: conversationId },
      include: { members: { include: { user: { select: { id: true, firstName: true, lastName: true, avatar: true, email: true } } } }, channel: true },
    });
  }
  ...
  async createMessage(payload: { userId: string; conversationId: string; content?: string; type?: string; replyToId?: string; mentions?: string[]; attachmentData?: Array<{ fileName: string; mimeType: string; size: number; storageKey: string; url: string }> }) {
    ...
    const message = await this.prisma.message.create({
      data: { conversationId: payload.conversationId, senderId: payload.userId, content, messageType: (payload.type ?? 'TEXT') as any, replyToId: payload.replyToId ?? null, mentions: payload.mentions?.length ? JSON.stringify(payload.mentions) : undefined },
      include: { sender: { select: { id: true, firstName: true, lastName: true, avatar: true } }, replyTo: { include: { sender: { select: { id: true, firstName: true, lastName: true, avatar: true } } } }, attachments: true },
    });

    if (payload.attachmentData?.length) {
      await this.prisma.messageAttachment.createMany({ data: payload.attachmentData.map((attachment) => ({ messageId: message.id, uploadedById: payload.userId, fileName: attachment.fileName, mimeType: attachment.mimeType, size: attachment.size, storageKey: attachment.storageKey, url: attachment.url })) });
    }
    ...
    if (this.notificationsService) {
      await Promise.allSettled([
        ...recipientIds.map((recipientId) => this.notifySafely(recipientId, { type: 'new-message', title: 'New message', message: `${message.sender.firstName ?? 'Someone'} sent a message`, data: { conversationId: payload.conversationId, messageId: message.id } })),
        ...mentionedUserIds.map((recipientId) => this.notifySafely(recipientId, { type: 'message-mention', title: 'You were mentioned', message: `${message.sender.firstName ?? 'Someone'} mentioned you in conversation`, data: { conversationId: payload.conversationId, messageId: message.id } })),
      ]);
    }

    return this.prisma.message.findUnique({
      where: { id: message.id },
      include: { sender: { select: { id: true, firstName: true, lastName: true, avatar: true } }, attachments: true, replyTo: { include: { sender: { select: { id: true, firstName: true, lastName: true, avatar: true } } } }, reactions: { include: { user: { select: { id: true, firstName: true, lastName: true, avatar: true } } } } },
    });
  }
  ...
  async toggleReaction(userId: string, messageId: string, reaction: string) {
    ...
    const existing = await this.prisma.messageReaction.findUnique({ where: { messageId_userId_reaction: { messageId, userId, reaction: normalizedReaction } } });
    let action: 'added' | 'removed';
    if (existing) {
      await this.prisma.messageReaction.delete({ where: { id: existing.id } });
      action = 'removed';
    } else {
      await this.prisma.messageReaction.create({ data: { messageId, userId, reaction: normalizedReaction } });
      action = 'added';
    }
    return { messageId, reaction: normalizedReaction, userId, action, reactions: await this.getReactionState(messageId) };
  }
