...
describe('AuthService', () => {
  ...
  it('falls back to a demo token when JWT signing cannot use the configured secret', async () => {
    prisma.user.findUnique.mockResolvedValueOnce(null);
    prisma.user.create.mockResolvedValueOnce({
      id: 'demo-id',
      email: 'demo@tribes.capital',
    ...
    const jwtService = { sign: jest.fn(() => { throw new Error('missing secret'); }) };
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AuthService,
        { provide: PrismaService, useValue: prisma },
    ...
    const result = await fallbackService.login({
      email: 'demo@tribes.capital',
      password: 'DemoPass123!',
    });
